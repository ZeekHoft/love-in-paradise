from flask import Flask, Response, stream_with_context, request, send_from_directory, redirect, jsonify
...
# Thread lock to prevent concurrent fact-checking requests
fact_check_lock = threading.Lock()
current_request = {"active": False, "user": None}
...
def handle_post_request():
    ...
    if not fact_check_lock.acquire(blocking=False):
        # Another request is in progress
        print(f"Request blocked - Another fact-check in progress")
        error_result = {
            "verdict": None,
            "justification": "Another fact-check is currently in progress. Please wait and try again.",
            "confidence": None,
            "sources": [],
            "currentProcess": "Busy",
            "progress": 0.0
        }
        return jsonify(error_result), 429  # Too Many Requests
    
    try:
        current_request["active"] = True
        current_request["user"] = request.remote_addr
        
        print(f"Received request from {request.remote_addr}: name={name}, useLLM={use_llm}")
        sys.stdout.flush()
        ...
        def generate():
            try:
                ...
                print(f"Request completed, lock released")
...
def check_status():
    return jsonify({
        "busy": current_request["active"],
        "processing_for": current_request["user"]
    })
